...
(function(mod) {
  ...
  TaysCodeMirror.defineMode("soy", function(config) {
    var textMode = TaysCodeMirror.getMode(config, "text/plain");
    var modes = {
      html: TaysCodeMirror.getMode(config, {name: "text/html", multilineTagIndentFactor: 2, multilineTagIndentPastTag: false}),
      attributes: textMode,
      text: textMode,
      uri: textMode,
      trusted_resource_uri: textMode,
      css: TaysCodeMirror.getMode(config, "text/css"),
      js: TaysCodeMirror.getMode(config, {name: "text/javascript", statementIndent: 2 * config.indentUnit})
    };
    ...
    return {
      startState: function() {
        return {
          ...
          localStates: [{
            mode: modes.html,
            state: TaysCodeMirror.startState(modes.html)
          }]
        };
      },
        ...
        return {
          ...
          localStates: state.localStates.map(function(localState) {
            return {
              ...
              state: TaysCodeMirror.copyState(localState.mode, localState.state)
            };
          })
        };
      },
            ...
            if (stream.match(/^\/?}/)) {
              ...
              if (state.context && state.context.tag == tagName && stream.current() == "kind" && (match = stream.match(/^="([^"]+)/, false))) {
                ...
                state.localStates.push({
                  mode: mode,
                  state: TaysCodeMirror.startState(mode)
                });
              }
              return "attribute";
            }
      ...
      indent: function(state, textAfter, line) {
        ...
        if (top == "comment") return TaysCodeMirror.Pass;
  ...
  TaysCodeMirror.registerHelper("wordChars", "soy", /[\w$]/);

  TaysCodeMirror.registerHelper("hintWords", "soy", Object.keys(tags).concat(
      ["css", "debugger"]));

  TaysCodeMirror.defineMIME("text/x-soy", "soy");
});
